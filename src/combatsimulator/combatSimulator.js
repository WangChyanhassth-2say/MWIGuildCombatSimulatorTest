...
class CombatSimulator extends EventTarget {
    ...
    async simulate(simulationTimeLimit) {
        ...
        if (this.zone?.isGuildTrial) {
            this.simResult.guildTrial = {
                encounterHrid: this.zone.hrid,
                startLevel: this.zone.startLevel,
                currentLevel: this.zone.currentLevel,
                maxLevel: this.zone.maxLevel,
                layersCompleted: this.zone.layersCompleted,
                points: this.zone.points,
                participantCount: this.zone.participantCount,
                hitpointMultiplier: this.zone.hitpointMultiplier,
                endedReason: this.zone.endedReason || (
                    this.simulationTime >= simulationTimeLimit ? "time_limit" : "stopped"
                ),
                remainingEnemies: (this.enemies ?? []).map((enemy) => ({
                    hrid: enemy.hrid,
                    currentHitpoints: enemy.combatDetails.currentHitpoints,
                    maxHitpoints: enemy.combatDetails.maxHitpoints,
                })),
                remainingPlayers: this.players.map((player) => ({
                    hrid: player.hrid,
                    currentHitpoints: player.combatDetails.currentHitpoints,
                    maxHitpoints: player.combatDetails.maxHitpoints,
                    currentManapoints: player.combatDetails.currentManapoints,
                    maxManapoints: player.combatDetails.maxManapoints,
                })),
            };
        }
    ...
    resetGuildTrialPlayersForNextLayer() {
        [
            AutoAttackEvent,
            AbilityCastEndEvent,
            AwaitCooldownEvent,
            ...
            FuryExpirationEvent,
        ].forEach((eventClass) => this.eventQueue.clearEventsOfType(eventClass.type));

        this.players.forEach((player) => {
            this.simResult.addRanOutOfManaCount(player, false, this.simulationTime);
            player.clearBuffs();
            player.clearCCs();
            player.resetCooldowns(this.simulationTime);
    ...
    checkParry(targets) {
        ...
        for (let attempt = 0; attempt < attempts; attempt++) {
            ...
            const parryUnit = parryUnits.splice(randomIndex, 1)[0];
    ...
    checkEncounterEnd() {
        ...
        if (this.enemies && !this.enemies.some((enemy) => enemy.combatDetails.currentHitpoints > 0)) {
            ...
            // this.eventQueue.clearEventsOfType(AbilityCastEndEvent.type);
            if (this.zone?.isGuildTrial) {
                const hasNextLayer = this.zone.completeLayer();
                if (hasNextLayer) {
                    this.resetGuildTrialPlayersForNextLayer();
            ...
            if (!this.zone?.isGuildTrial || !this.trialEnded) {
                this.enemies = null;
            }
        ...
        this.players.forEach(player => {
            if ((player.combatDetails.currentHitpoints <= 0) && !this.eventQueue.containsEventOfTypeAndHrid(PlayerRespawnEvent.type, player.hrid)) {
                if (this.zone && !this.zone.isDungeon && !this.zone.isGuildTrial) {
        ...
        if (
            ...
            if (this.zone?.isGuildTrial) {
                this.zone.endedReason = "party_wiped";
                this.trialEnded = true;
                this.eventQueue.clear();
                encounterEnded = true;
